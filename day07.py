...
def does_it_contain(bag, bag_mine):
    if bag_mine in rules[bag].keys():
        ...
        return True
    elif 'no other' in rules[bag].keys():
        return False
    else:
        if any(does_it_contain(bag_contained, bag_mine) for bag_contained in rules[bag].keys()):
            return True
        else:
            return False
...
for line in lines:
    ...
    rules[bag_outer] = bag_inner_dict
...
def bags_it_contains(bag):
    count = 0
    if 'no other' in rules[bag].keys():
        return count
    else:
        for num_contained in rules[bag].values():
            count += num_contained
        for bag_contained in rules[bag].keys():
